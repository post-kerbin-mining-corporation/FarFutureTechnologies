...
namespace FarFutureTechnologies
{
    public class ModuleMultiRatioEngine: PartModule
    {
      ...
      [KSPField(isPersistant = true, guiActive = true, guiName = "Mixing Ratio"), UI_FloatRange(minValue = 0.1f, maxValue = 3.5f, stepIncrement = 10f)]
      ...
      // Relates mixing ratio to Isp
      [KSPField(isPersistant = false)]
      public FloatCurve IspCurveScale;
      ...
      public override string GetInfo()
      {
        string msg = Localizer.Format("#LOC_FFT_ModuleMultiRatioEngine_PartInfo",
          (IspCurve.Evaluate(0f)*IspCurveScale.Evaluate(IspCurve.maxTime)).ToString("F2"),
          (IspCurve.Evaluate(0f)*IspCurveSacle.Evaluate(IspCurve.minTime)).ToString("F2"),
          ThrustCurve.Evaluate(IspCurve.maxTime).ToString("F2"),
          ThrustCurve.Evaluate(IspCurve.minTime).ToString("F2"));
        return msg;
      }
      ...
      void HandleMixingRatioChange()
      {
          ...
          atmosphereCurve.Add(IspCurve.Evaluate(0f)* ispScale);
          atmosphereCurve.Add(IspCurve.Evaluate(1f)* ispScale);
          atmosphereCurve.Add(IspCurve.Evaluate(4f)* ispScale);
